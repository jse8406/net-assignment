...
fn main() -> std::io::Result<()> {
    ...
    loop {
        ...
        let msg_to_send = match choice {
            ...
            "5" => {
                    // Send "OPT5" to server before exiting
            let _ = stream.lock().unwrap().write_all(b"OPT5");
            // Shutdown the stream
        ...
        let write_result = stream.lock().unwrap().write_all(msg_to_send.as_bytes());
        ...
        let size = match stream.lock().unwrap().read(&mut buffer) {
            Ok(0) => {
